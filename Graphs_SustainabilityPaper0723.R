...
Corr_In<-select(LaoSDG_ALL_LC,PovChange,`PovChange 1T 10%`,StuntChange, Pov05, `StuntChangeSE 1T 10%`, Stunt05, PerNatural05, NatChange) #Final one
...
p3<-ggplot(LaoSDG_ALL_LC, aes(x=PerNatural05, y=-NatChange))+ #Final
  ...
  xlab("Natural Area 2005") + ylab("Natural Area Reduction (p.p.)")+
...
sankeyNetwork(Links = LaoSDG_LC_change, Nodes = nodes,
              ...
              fontSize= 14,  width = 500, height= 500,
...
# Appendix G.1: Number of countries (800x500)
...
#### Appendix G.3: Box plots (1200x400)
...
p2<-ggplot(LaoSDG_ALL_LC[(LaoSDG_ALL_LC$tot_popgr<0.5 & LaoSDG_ALL_LC$tot_popgr>-0.2),], aes(x=tot_popgr, y=PovChange))+ #Pov vs Nat
  ...
  scale_colour_discrete(name="Significance Poverty",labels=c("No","Yes"))+
  theme(legend.position = "bottom")+
  labs(title = paste("B. Corr =",signif(Corr_Pp$r[1,2], 2),
...
p3<-ggplot(LaoSDG_ALL_LC[(LaoSDG_ALL_LC$tot_popgr<0.5 & LaoSDG_ALL_LC$tot_popgr>-0.2),], aes(x=tot_popgr, y=NatChange))+ #Stunt vs Nat
  ...
  scale_colour_manual(name="Increase agri. land area",values=c("turquoise3","red", "blue"), labels=c("Normal","Low","High"))+ #name="Land Use Type",
  theme(legend.position = "bottom")+
  labs(title = paste("A. Corr =",signif(Corr_Np$r[1,2], 2),
...
rm(list = ls(pattern = "^Corr"))
rm(list = ls(pattern = "^p"))
rm(LaoSDG_ALL_LC,PovStunt_2000_OWD)
